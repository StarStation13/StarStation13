...
/obj/machinery/pattern_table/proc/create_pattern(mob/living/user, list/modifiers)
	...
	if(!has_dmi || has_dmi == "No")
		...
		to_chat(user, "Use those names exactly. Furthermore, Clothing .dmi files must be 32x32.")
		to_chat(user, "Any uploaded clothes are subject to admin removal at their discretion.")
		return
